...
const CREATE_STATEMENT = 'CREATE  TABLE IF NOT EXISTS `sessions` (`id` VARCHAR(100) NOT NULL, `expires` BIGINT NULL, `data` TEXT NULL, PRIMARY KEY (`id`));'
    , GET_STATEMENT = 'SELECT * FROM `sessions` WHERE id  = ?'
    , SET_STATEMENT ='INSERT INTO sessions(id, expires, data) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE expires=?, data =?'
    , DELETE_STATEMENT = 'DELETE FROM `sessions` WHERE id  = ?'
    , CLEANUP_STATEMENT = 'DELETE FROM `sessions` WHERE expires  < ?' ;
